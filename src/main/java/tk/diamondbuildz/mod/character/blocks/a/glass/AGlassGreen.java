...
public class AGlassGreen extends BlockBaseGlassA {
    public Item item;
    public EntityPlayer entityPlayer;
    public AGlassGreen(@Nonnull String name) {
        super(name);
    }
    @Override
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        if (harvesters.get() != null) {
            ...
            if (MainHandHeldItem.equals(ModItems.TOOL_DIAMOND_GLASS_CUTTER) || MainHandHeldItem.equals(ModItems.TOOL_IRON_GLASS_CUTTER)) {
                this.item = Item.getItemFromBlock(ModBlocks.A_GLASS_GREEN);
            }
            else {
                this.item = ModItems.GLASS_SHARD_GREEN;
            }
        }
        return this.item;
    }
    ...
    protected ItemStack getSilkTouchDrop(IBlockState state) {
        return new ItemStack(ModBlocks.A_GLASS_GREEN);
    }
