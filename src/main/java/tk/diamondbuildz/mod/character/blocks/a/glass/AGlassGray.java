...
public class AGlassGray extends BlockBaseGlassA {
    public Item item;
    public EntityPlayer entityPlayer;
    public AGlassGray(@Nonnull String name) {
        super(name);
    }
    @Override
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        if (harvesters.get() != null) {
            ...
            if (MainHandHeldItem.equals(ModItems.DIAMOND_GLASS_CUTTER) || MainHandHeldItem.equals(ModItems.IRON_GLASS_CUTTER)) {
                this.item = Item.getItemFromBlock(ModBlocks.A_GLASS_GRAY);
            }
            else {
                this.item = ModItems.GLASS_SHARD_GRAY;
            }
        }
        return this.item;
    }
    ...
    protected ItemStack getSilkTouchDrop(IBlockState state) {
        return new ItemStack(ModBlocks.A_GLASS_GRAY);
    }
